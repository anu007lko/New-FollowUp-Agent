...
for m in facts.messages:
    print(f"  Sender: {m.sender_email} | Direction: {m.direction} | Meaningful: {m.is_meaningful} | Body: {m.body_preview[:60]}")

evaluate_interview_status(facts, datetime.now(timezone.utc))
